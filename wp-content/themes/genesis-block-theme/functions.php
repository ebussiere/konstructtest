		...
		*/
		...
		register_nav_menus(
			array(
				...
				'social'  => esc_html__( 'Social Icon Menu', 'genesis-block-theme' ),
			) 
		);
...
if ( ! function_exists( 'genesis_block_theme_fonts_url' ) ) :
	...
	function genesis_block_theme_fonts_url() {
		...
		if ( 'off' !== $font ) {
			$font_family = 'Public Sans:ital,wght@0,100..900;1,100..900';

			$query_args = array(
				'family' => rawurlencode( $font_family ),
				'subset' => rawurlencode( 'latin,latin-ext' ),
			);

			$fonts_url = add_query_arg( $query_args, 'https://fonts.googleapis.com/css2?&display=swap' );
		}
...
function genesis_block_theme_scripts() {
	...
	wp_enqueue_style( 'genesis-block-theme-style', get_stylesheet_uri() );

	/**
	* Load fonts from Google
	*/
	wp_enqueue_style( 'genesis-block-theme-fonts', genesis_block_theme_fonts_url(), array(), null );
	 ...
	 */
	wp_enqueue_script( 'genesis-block-theme-js', get_template_directory_uri() . '/js/genesis-block-theme.js', array( 'jquery' ), '1.0', true );
...
function genesis_block_theme_admin_scripts( $hook ) {
	...
	*/
	wp_enqueue_style( 'genesis-block-theme-admin-fonts', genesis_block_theme_fonts_url(), array(), null );
...
function genesis_block_theme_customizer_styles_for_block_editor() {
	 ...
	 */
	wp_register_style( 'genesis-block-theme-customizer-styles', false );
